...
def training_loop(agent, envs, args, device, actor_critic,
    ...
    if args.birthx_linear_tc_steps >= 0:
        ...
        utils.validate_tc_schedule_datasets(schedule, args.dataset, args.loc_algo) # fail fast on dataset-name/loc_algo mismatches that would silently no-op
    ...
    # ou_stage_transitions are stored as global indices after the schedule key shift above.
    ...
    for j in update_range:
        j_global = j + stage_offset   # absolute step for wandb logging; j is used for save/log intervals
