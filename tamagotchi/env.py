...
class PlumeEnvironment_v3(gym.Env):
    def __init__(self, flip_ventral_optic_flow=False,
        ...
        if self.walking:
            raise ValueError("walking is not supported by PlumeEnvironment_v3 - wind advection is part "
                             "of the action physics. Set walking=False.")
    ...
    def sample_rotate_by(self):
        ...
        This is used to rotate the data by a random angle and mirror it along the long axis of the plume.
        This is to help generalization.
        """
    ...
    def get_current_wind_xy(self):
        ...
        if self.rotate_by:
            df = rotate_wind_optimized(self.data_wind.loc[df_idx], self.rotate_by, self.mirror) # Rotate wind by self.rotate_by degrees
    ...
    def _reset_base(self):
      """Core episode reset: sample start time/location/angle, subset puff data,
      apply per-episode randomizations, and return the first observation.
      """
...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, args=None):
    def _thunk():
        env_version = getattr(args, 'env_version', 'v3')
        if env_version != 'v3':
            raise ValueError(f"only env_version='v3' is supported, got {env_version!r}")
